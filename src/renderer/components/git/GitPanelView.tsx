...
export interface GitPanelViewProps {
  ...
  repoPath: string
  /** Branch to pin in the compare picker (e.g. session.baseBranch). Optional. */
  baseBranch?: string
  /** Session ID — when provided, PR preview selection is persisted per session. */
  sessionId?: string
  /** Whether this panel is the visible/active tab. Polling pauses when false. */
  visible: boolean
}

export function GitPanelView({ repoPath, baseBranch, sessionId, visible }: GitPanelViewProps) {
  ...
  // Poll only when this panel is visible. Multiple GitPanelView instances may
  // be mounted (one per workspace) — only the active one should drive the
  // shared useGitStore.
  useEffect(() => {
    if (!repoPath) {
      clear()
  ...
  // Save/restore PR preview when session changes (only when scoped to a session)
  useEffect(() => {
    if (!sessionId) return
    const savedBranch = getSavedBranchForSession(sessionId)
    if (savedBranch) {
      usePRPreviewStore.getState().activate(repoPath, savedBranch, sessionId)
    } else {
  ...
  const handleOpenPR = async () => {
    ...
    setOpeningPR(true)
    try {
      await window.api.git.openPR(repoPath)
    } catch (err: any) {
      addToast('error', err.message)
    } finally {
      setOpeningPR(false)
  ...
  const handleCompareSelect = useCallback((branch: string | null) => {
    if (!repoPath) return
    if (branch === null) {
      usePRPreviewStore.getState().deactivate(sessionId)
    } else {
      usePRPreviewStore.getState().activate(repoPath, branch, sessionId)
    }
  }, [repoPath, sessionId])
  ...
  if (!repoPath) {
    return (
      <div className="flex-1 flex items-center justify-center text-text-muted text-xs">
        Select a session or project to view git history
      </div>
    )
  }
    ...
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex items-center gap-1 bg-bg-secondary border-b border-border" style={{ padding: '2px 4px' }}>
        ...
        <BranchPickerDropdown
          ...
          sessionBaseBranch={baseBranch}
          onSelect={handleCompareSelect}
        />
      </div>

      {previewActive ? (
        <PRPreviewPanel repoPath={repoPath} />
      ) : (
        <>
          {/* Three-column layout */}
          <div className="flex-1 flex min-h-0 min-w-0">
            ...
            <div style={{ width: filesCol.size }} className="flex-shrink-0 flex flex-col min-h-0">
              ...
              <ChangedFiles repoPath={repoPath} />
            </div>
