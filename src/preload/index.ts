...
const api = {
  git: {
    ...
    showFile: (repoPath: string, ref: string, filePath: string): Promise<string | null> =>
      ipcRenderer.invoke(IPC.GIT_SHOW_FILE, repoPath, ref, filePath),
  ...
  worktree: {
    ...
    renameBranch: (repoPath: string, oldBranch: string, newBranch: string): Promise<void> =>
      ipcRenderer.invoke(IPC.WORKTREE_RENAME_BRANCH, repoPath, oldBranch, newBranch),
  },
