...
export function registerSchedulerHandlers(window: BrowserWindow) {
  ...
  handle(IPC.SCHEDULER_LIST_QUEUED_SESSIONS, async () => {
    return scheduler.listQueuedSessions()
  })

  handle(IPC.SCHEDULER_ADD_QUEUED_SESSION, async (_e, item: QueuedSession) => {
    return scheduler.addQueuedSession(item)
  })

  handle(IPC.SCHEDULER_CANCEL_QUEUED_SESSION, async (_e, id: string) => {
  ...
  handle(IPC.SCHEDULER_FIRE_QUEUED_SESSION_NOW, async (_e, id: string) => {
    scheduler.fireQueuedSessionNow(id)
  })

  handle(IPC.SCHEDULER_LIST_QUEUED_MESSAGES, async () => {
    return scheduler.listQueuedMessages()
  })

  handle(IPC.SCHEDULER_ADD_QUEUED_MESSAGE, async (_e, item: QueuedMessage) => {
    return scheduler.addQueuedMessage(item)
  })

  handle(IPC.SCHEDULER_CANCEL_QUEUED_MESSAGE, async (_e, id: string) => {
